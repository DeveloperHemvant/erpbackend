...
async function main() {
  ...
  if (!students.length || !staff.length || !campuses.length || !enrollments.length || !rooms.length || !vehicles.length) {
    console.error('Core data not found. Run npm run seed first!');
    return;
  }
  ...
  await safeSeed('Teacher Substitutions', async () => {
    const activeLeaves = await prisma.leaveApplication.findMany({ select: { id: true, staffId: true } });
