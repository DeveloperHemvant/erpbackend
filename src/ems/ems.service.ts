...
export class EmsService {
  ...
  private async getEnrolledStudentsForClass(classId: string) {
    const enrollments =
      await this.emsRepository.findEnrolledStudentsForClass(classId);
  ...
  async createQuestion(questionBankId: string, data: CreateQuestionDto) {
    return this.emsRepository.createQuestion({
      questionBankId,
      ...data,
  ...
  async addQuestionToPaper(
    ...
    dto: AddQuestionPaperItemDto,
  ) {
    const [question] = await this.emsRepository.findQuestionsByIds([
      dto.questionId,
    ]);
    if (!question) throw new NotFoundException('Question not found');

    const sequence =
      (await this.emsRepository.countQuestionPaperItems(questionPaperId)) + 1;
  ...
  async activateOnlineSchedule(
    scheduleId: string,
    dto: ActivateOnlineScheduleDto,
  ) {
    ...
    const paper = await this.emsRepository.findQuestionPaperById(
      dto.questionPaperId,
    );
    if (!paper) throw new NotFoundException('Question paper not found');
    if (!paper.isApproved)
      throw new BadRequestException(
        'Question paper must be approved before it can be scheduled online',
      );
    if (paper.items.length === 0)
      throw new BadRequestException('Question paper has no questions');
  ...
  async startOnlineAttempt(attemptId: string, user: AuthenticatedUser) {
    ...
    if (attempt.schedule.mode !== 'ONLINE')
      throw new BadRequestException('This exam is not an online test');
    if (!attempt.schedule.questionPaperId)
      throw new BadRequestException(
        'No question paper attached to this schedule',
      );

    const existing =
      await this.emsRepository.findOnlineSubmissionByAttempt(attemptId);
    if (existing) {
    ...
    await this.emsRepository.updateAttemptTiming(
      attemptId,
  ...
  async getAttemptPaper(attemptId: string, user: AuthenticatedUser) {
    ...
    if (!submission)
      throw new BadRequestException(
        'Start the test before requesting questions',
      );
    if (await this.closeIfExpired(submission)) {
      throw new BadRequestException(
        'Time is up — this test has been auto-submitted',
      );
    }
    if (submission.status !== 'IN_PROGRESS')
      throw new BadRequestException('This test is not in progress');

    const schedule = await this.emsRepository.findScheduleWithQuestionPaper(
      attempt.scheduleId,
    );
    if (!schedule?.questionPaper)
      throw new NotFoundException('Question paper not found for this schedule');

    const answersByQuestion = new Map(
      submission.answers.map((a) => [a.questionId, a]),
    );
    const deadline = new Date(
      submission.startedAt!.getTime() + (schedule.durationMin ?? 0) * 60000,
    );
    ...
    return {
      ...
      questions: schedule.questionPaper.items.map((item) => ({
        ...
        selectedOptionId:
          answersByQuestion.get(item.questionId)?.selectedOptionId ?? null,
      })),
    };
  }
  ...
  async saveAnswer(
    ...
    await this.loadOwnedAttempt(attemptId, user);

    const submission =
      await this.emsRepository.findOnlineSubmissionByAttempt(attemptId);
    if (!submission)
      throw new BadRequestException('Start the test before answering');
    if (await this.closeIfExpired(submission)) {
      throw new BadRequestException(
        'Time is up — this test has been auto-submitted',
      );
    ...
    if (submission.status !== 'IN_PROGRESS')
      throw new BadRequestException('This test is no longer accepting answers');
  ...
  async submitOnlineAttempt(attemptId: string, user: AuthenticatedUser) {
    ...
    const submission =
      await this.emsRepository.findOnlineSubmissionByAttempt(attemptId);
    if (!submission)
      throw new BadRequestException('This test was never started');
    if (submission.status !== 'IN_PROGRESS')
      throw new BadRequestException('This test has already been submitted');
  ...
  private async closeIfExpired(submission: {
    id: string;
    scheduleId: string;
    startedAt: Date | null;
    status: string;
  }) {
    if (submission.status !== 'IN_PROGRESS' || !submission.startedAt)
      return false;
    const schedule = await this.emsRepository.findScheduleById(
  ...
  private async sweepExpiredSubmissions() {
    ...
    for (const submission of inProgress) {
      const deadline =
        (submission.startedAt?.getTime() ?? 0) +
        (submission.schedule.durationMin ?? 0) * 60000;
      if (now > deadline) {
        await this.gradeAndCloseSubmission(submission.id, true);
      }
    }
  ...
  private async gradeAndCloseSubmission(
    submissionId: string,
    autoSubmitted: boolean,
  ) {
    const submission =
      await this.emsRepository.findOnlineSubmissionById(submissionId);
    ...
    for (const item of items) {
      ...
      const isCorrect =
        ...
        answer.selectedOptionId === item.question.correctOptionId;
    ...
    await this.emsRepository.createEvaluation({
      ...
      remarks: autoSubmitted
        ? 'Auto-graded (online MCQ, time expired)'
        : 'Auto-graded (online MCQ)',
    });
  ...
  async generateAttempts(scheduleId: string, dto: GenerateAttemptsDto) {
    ...
    const existing = await this.emsRepository.findAttemptStudentIds(
      scheduleId,
      students.map((s) => s.id),
    );
  ...
  async getAttempts(scheduleId: string) {
    return this.emsRepository.findAttemptsForScheduleWithEvaluations(
      scheduleId,
    );
  }
  ...
  async createGradingScheme(data: CreateGradingSchemeDto) {
    return this.emsRepository.createGradingScheme({
      ...data,
      ranges: data.ranges as any,
    });
  }
  ...
  async computeResults(gradebookId: string, dto: ComputeResultsDto) {
    ...
    const schedules = await this.emsRepository.findSchedulesBySession(
      gradebook.sessionId,
    );
    const totalPossible = schedules.reduce(
      (sum, s) => sum + s.template.totalMarks,
      0,
    );

    let scheme: { ranges: any } | null = null;
    if (dto.gradingSchemeId) {
      scheme = await this.emsRepository.findGradingSchemeById(
        dto.gradingSchemeId,
      );
    }
    const ranges = (scheme?.ranges as {
      min: number;
      max: number;
      grade: string;
    }[]) || [
    ...
    const scored = await Promise.all(
      students.map(async (student) => {
        const evaluations =
          await this.emsRepository.findEvaluationsForStudentSession(
            student.id,
            gradebook.sessionId,
          );
    ...
    const existingResults =
      await this.emsRepository.findResultsForGradebook(gradebookId);
    ...
    const results = await Promise.all(
      scored.map((s, index) => {
        ...
        return this.emsRepository.createResult({
          gradebookId,
  ...
  async publishGradebook(id: string) {
    ...
    this.notifyParentsOfStudents(
      ...
      console.error('Failed to notify parents of published results', err),
    );
    return gradebook;
  }
  ...
  private async notifyParentsOfStudents(
    ...
    const students =
      await this.emsRepository.findStudentsWithParents(studentIds);
  ...
  async generateReportCard(data: GenerateEmsReportCardDto) {
    const results = await this.emsRepository.findResultsForReportCard(
      data.studentId,
      data.sessionId,
    );
    const summary =
    ...
    this.notifyParentsOfStudents(
      ...
      console.error('Failed to notify parents of report card', err),
    );
  ...
  async getStudentExams(studentId: string) {
    ...
    const seatings = scheduleIds.length
      ? await this.emsRepository.findSeatingsForStudentSchedules(
          studentId,
          scheduleIds,
        )
      : [];
    const seatByScheduleId = new Map(
      seatings.map((s) => [s.room.scheduleId, s]),
    );

    const submissions = attempts.length
      ? await this.emsRepository.findSubmissionsForAttempts(
          attempts.map((a) => a.id),
        )
      : [];
    const submissionByAttempt = new Map(
      submissions.map((s) => [s.attemptId, s]),
    );
      ...
      return {
        ...
        seat: seat
          ? {
              ...
              invigilators: seat.room.invigilators.map(
                (iv) => iv.staff.fullName,
              ),
            }
          : null,
      };
  ...
  async getStudentResults(studentId: string) {
    ...
    const reportCards =
      await this.emsRepository.findReportCardsForStudent(studentId);
  ...
  async getRoomAttendance(roomId: string) {
    ...
    const attempts = studentIds.length
      ? await this.emsRepository.findAttemptsByScheduleAndStudents(
          room.scheduleId,
          studentIds,
        )
      : [];
  ...
  async getStaffEvaluationQueue(staffId: string) {
    ...
    const subjectIds = Array.from(
      new Set(
        assignments.map((a) => a.subjectId).filter((id): id is string => !!id),
      ),
    );
    if (subjectIds.length === 0) return { pending: [], graded: [] };

    const attempts =
      await this.emsRepository.findAttemptsForSubjects(subjectIds);
