...
export class StaffController {
  ...
  @ApiResponse({ status: 200, description: "Staff roster retrieved successfully" })
  ...
  @RequirePermissions()
  @ApiOperation({ summary: "Get staff profile by UUID — the staff member's own record, or any record for admins" })
  ...
  markSelfAttendance(@Body() data: { staffId: string, status: string, date: string }) {
    return this.staffService.markSelfAttendance(data);
  }

  @Get('self-attendance/:staffId')
  @RequirePermissions()
  @ApiOperation({ summary: "Check if logged in staff has already marked attendance today" })
  getSelfAttendance(@Param("staffId", ParseUUIDPipe) staffId: string, @Query("date") date?: string) {
  ...
  getAttendanceLogs(@Param("id", ParseUUIDPipe) id: string, @Query("month") month?: string) {
  ...
  @ApiParam({ name: "id", format: "uuid" })
  uploadFile(@Param("id", ParseUUIDPipe) id: string, @UploadedFile() file: any) {
