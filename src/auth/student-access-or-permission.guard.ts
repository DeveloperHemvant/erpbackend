...
import { STUDENT_ACCESS_OR_PERMISSION_KEY, StudentAccessOrPermissionConfig } from "./student-access-or-permission.decorator";
...
export class StudentAccessOrPermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const config = this.reflector.getAllAndOverride<StudentAccessOrPermissionConfig>(STUDENT_ACCESS_OR_PERMISSION_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    if (permissions.includes("*") || config.permissions.some((p) => permissions.includes(p))) return true;
