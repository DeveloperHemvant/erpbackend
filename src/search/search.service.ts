...
const SEARCHERS: Record<string, (repo: SearchRepository, q: string, limit: number) => Promise<SearchResult[]>> = {
...
export class SearchService {
  ...
  async search(q: string, user: AuthenticatedUser, limit = 20): Promise<SearchResult[]> {
    if (!q || q.trim().length < 2) return [];

    const isSuperAdmin = user.permissions.includes("*");
    const allowedTypes = Object.keys(SEARCHERS).filter(
      (type) => isSuperAdmin || user.permissions.includes(ENTITY_VIEW_PERMISSION[type]),
    );

    const perTypeLimit = Math.max(3, Math.ceil(limit / Math.max(allowedTypes.length, 1)));
    const results = await Promise.all(
      allowedTypes.map((type) => SEARCHERS[type](this.repository, q, perTypeLimit).catch(() => [])),
    );
