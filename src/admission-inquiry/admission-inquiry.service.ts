...
export class AdmissionInquiryService {
  ...
  async addFollowUp(inquiryId: string, staffId: string, dto: AddFollowUpDto) {
    ...
    if (!existing) throw new NotFoundException("Inquiry not found");
    return this.repository.addFollowUp({ inquiryId, createdByStaffId: staffId, note: dto.note });
  }

  async convert(id: string, dto: ConvertInquiryDto) {
    const existing = await this.repository.findById(id);
    if (!existing) throw new NotFoundException("Inquiry not found");
    if (existing.convertedStudentId) throw new BadRequestException("This inquiry has already been converted");
    return this.repository.update(id, { status: "Converted", convertedStudentId: dto.studentId });
  }
}
