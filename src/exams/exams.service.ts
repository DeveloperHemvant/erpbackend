...
export class ExamsService {
  ...
  async createExamSlot(dto: CreateExamSlotDto) {
    ...
    const existing = await this.examRepository.findClashingSlot(
      dto.date,
      dto.room,
      dto.startTime,
      dto.endTime,
    ...
    if (existing) {
      throw new BadRequestException(
        `Room ${dto.room} is already booked for another exam at this time.`,
      );
    }
  ...
  async submitExamMarks(dto: SubmitExamMarksDto) {
    ...
    for (const mark of dto.marks) {
      // Upsert marks
      const existing = await this.examRepository.findExamMarkByEnrollment(
        dto.examSlotId,
        mark.enrollmentId,
      );
      if (existing) {
        await this.examRepository.updateExamMark(
          existing.id,
          mark.marksObtained,
          mark.isAbsent,
        );
      } else {
        await this.examRepository.createExamMark(
          dto.examSlotId,
          mark.enrollmentId,
          mark.marksObtained,
          mark.isAbsent,
