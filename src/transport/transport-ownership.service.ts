...
export class TransportOwnershipService {
  ...
  async assertVehicleAccess(
    ...
    const assignment =
      await this.transportRepository.findVehicleStaffAssignment(
        user.userId,
        vehicleId,
      );
    if (!assignment) {
  ...
  async assertRouteAccess(
    ...
    if (!route.vehicleId) {
      ...
      throw new ForbiddenException(
        'This route has no assigned vehicle — only a transport manager can edit it.',
      );
    }
    await this.assertVehicleAccess(user, route.vehicleId);
  }

  async assertStopAccess(
    user: AuthenticatedUser,
    stopId: string,
  ): Promise<void> {
    if (this.hasFleetAccess(user)) return;
