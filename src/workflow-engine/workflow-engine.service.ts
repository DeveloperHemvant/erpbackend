...
export class WorkflowEngineService implements OnModuleInit {
  ...
  async onModuleInit() {
    await this.repository.upsert("applicant", "Admissions Pipeline", {
      ...
      stages: ["New", "Contacted", "Campus Visit Scheduled", "Application Sent", "Converted", "Lost"],
      transitions: [
  ...
  async validateTransition(entityType: string, fromStage: string, toStage: string) {
    const definition = await this.repository.findByEntityType(entityType);
    if (!definition) {
      return { valid: false, reason: `No workflow definition registered for entity type "${entityType}".` };
    }
    ...
    const legal = transitions.some((t) => t.from === fromStage && t.to === toStage);
    return legal
      ? { valid: true }
      : { valid: false, reason: `"${fromStage}" → "${toStage}" is not a legal transition for ${definition.name}.` };
  }
}
