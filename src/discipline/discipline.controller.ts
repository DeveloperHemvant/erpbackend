...
import { CreateDisciplineIncidentDto, UpdateDisciplineIncidentDto, AddCounselingNoteDto } from "./dto/discipline.dto";
...
export class DisciplineController {
  ...
  @RequirePermissions()
  getIncidentsForStudent(@Param("studentId") studentId: string, @CurrentUser() user: AuthenticatedUser) {
    const isStaff = user.permissions.includes("*") || user.permissions.includes("MANAGE_DISCIPLINE");
    return isStaff
  ...
  @Patch("incidents/:id")
  updateIncident(@Param("id") id: string, @Body() dto: UpdateDisciplineIncidentDto) {
    return this.disciplineService.updateIncident(id, dto);
  }

  @Post("incidents/:id/notes")
  addCounselingNote(@Param("id") id: string, @Body() dto: AddCounselingNoteDto, @CurrentUser() user: AuthenticatedUser) {
