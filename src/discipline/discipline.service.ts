...
export class DisciplineService {
  ...
  async createIncident(studentId: string, staffId: string, dto: CreateDisciplineIncidentDto) {
    ...
    if (notifyParent) {
      ...
      this.communicationService.sendCustomAlert(studentId, title, body).catch((err) => console.error("Failed to notify parent of discipline incident", err));
    }
  ...
  async addCounselingNote(incidentId: string, staffId: string, dto: AddCounselingNoteDto) {
